...
public class ExtractWallArea : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        List<string> csvLines = new List<string> {
            "Rm. No.,Room Name,Wall Id,Wall GUID,Wall Length (ft),Wall Area (sqft),Wall Orientation"
        };
                ...
                if (room.Location is LocationPoint locationPoint && room.Area > 0)
                {
                    ...
                    foreach (var boundaryList in boundaries)
                    {
                        foreach (BoundarySegment seg in boundaryList)
                        {
                            ...
                            // GUID
                            string guid = wall.GetEntitySchemaGuids().ToString();

                            csvLines.Add($"{room.Number},{room.Name},{wall.Id},{guid},{curve.Length:F2},{area:F2},{orientation},");
                        }
                    }
                }
            }
